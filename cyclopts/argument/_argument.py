...
from cyclopts.utils import UNSET, apply_name_transform, grouper, is_builtin, parse_version
...
class Argument:
    ...
    **Python** keys that lead to this leaf.
    ...
    _transform_secondary_names: tuple[str, ...] = field(
        alias="transform_secondary_names", default=(), repr=False, eq=False
    )
    """Names generated by ``name_transform`` at index >= 1 (subject to first-wins collision dropping)."""
        ...
        while i < len(trailing):
            ...
            for canonical_name, fi in field_infos.items():
                for alias in fi.names:
                    for transformed in apply_name_transform(name_transform, alias):
                        name_map.setdefault(transformed, (canonical_name, fi.annotation))
