...
from cyclopts.utils import UNSET, apply_name_transform, is_iterable
...
class ArgumentCollection(list[Argument]):
    ...
    def _resolve_transform_name_collisions(self) -> None:
        """Drop ``name_transform``-generated secondary names that collide with already-claimed names.

        Collision policy is **first-wins**: a secondary (index >= 1) name generated by an
        earlier argument claims the name; later arguments silently lose colliding secondaries.
        Canonical/explicit-name collisions are left untouched (status quo).
        """
        claimed: set[str] = set()
        for argument in self:
            secondary = set(argument._transform_secondary_names)
            if secondary:
                assert isinstance(argument.parameter.name, tuple)
                kept: list[str] = []
                for name in argument.parameter.name:
                    if name in secondary and (name in claimed or name in kept):
                        continue  # silently drop generated alias
                    kept.append(name)
                if len(kept) != len(argument.parameter.name):
                    argument.parameter = Parameter.combine(argument.parameter, Parameter(name=tuple(kept)))
                    argument._transform_secondary_names = tuple(n for n in kept if n in secondary)
            claimed.update(argument.names)  # argument.names includes derived negatives
    ...
    def _from_type(
        ...
        *default_parameters: Parameter | None,
        ...
        transform_secondary_names: tuple[str, ...] = ()
        ...
        if keys:
            ...
            if immediate_parameter.name:
                child_names = immediate_parameter.name
            else:
                primary_child: list[str] = []
                secondary_child: list[str] = []
                for x in field_info.names:
                    for i, transformed in enumerate(apply_name_transform(cparam.name_transform, x)):
                        (primary_child if i == 0 else secondary_child).append(transformed)
                child_names = tuple(primary_child) + tuple(secondary_child)
                # A name reachable as a primary (or alias) must never be marked secondary.
                resolved_primary = set(
                    resolve_parameter_name(upstream_parameter.name, tuple(primary_child) + cparam.alias)  # pyright: ignore
                )
                transform_secondary_names = tuple(
                    dict.fromkeys(
                        name
                        for name in resolve_parameter_name(upstream_parameter.name, tuple(secondary_child))  # pyright: ignore
                        if name not in resolved_primary
                    )
                )
            cparam = Parameter.combine(
                cparam,
                Parameter(
                    name=resolve_parameter_name(
                        ...
                        child_names + cparam.alias,  # pyright: ignore
                    )
                ),
            )
        else:
            ...
            if cparam.name:
                ...
                if field_info.kind in (field_info.POSITIONAL_ONLY, field_info.VAR_POSITIONAL):
                    ...
                    for name in field_info.names:
                        for i, transformed in enumerate(apply_name_transform(cparam.name_transform, name)):
                            cli_name = transformed if transformed.startswith("-") else "--" + transformed
                            (primary_names if i == 0 else secondary_names).append(cli_name)
                    # Dedupe exact duplicates (keep first); primaries take priority over secondaries.
                    seen: set[str] = set()
                    generated_names: list[str] = []
                    secondary_final: list[str] = []
                    for cli_name in primary_names:
                        if cli_name not in seen:
                            seen.add(cli_name)
                            generated_names.append(cli_name)
                    for cli_name in secondary_names:
                        if cli_name not in seen:
                            seen.add(cli_name)
                            generated_names.append(cli_name)
                            secondary_final.append(cli_name)
                    transform_secondary_names = tuple(secondary_final)
                    cparam = Parameter.combine(
                        cparam,
                        Parameter(name=tuple(generated_names) + resolve_parameter_name(cparam.alias)),
                    )
        ...
        argument = Argument(
            ...
            keys=keys,
            hint=hint,
            transform_secondary_names=transform_secondary_names,
        )
        ...
        if argument._accepts_keywords:
            if transform_secondary_names:
                # Transform-generated secondary names are leaf-level aliases only; they must not
                # become dotted namespace prefixes for child parameters (unlike explicit name/alias).
                # Children are built *before* collection-wide collision resolution, so a propagated
                # prefix could outlive a secondary that later gets dropped.
                assert isinstance(cparam.name, tuple)
                cparam = Parameter.combine(
                    cparam,
                    Parameter(name=tuple(name for name in cparam.name if name not in transform_secondary_names)),
                )
    ...
    def _from_callable(
        ...
        *default_parameters: Parameter | None,
        ...
        if _resolve_groups:
            # Skip during the throwaway group-discovery pass (``_resolve_groups=False``).
            out._resolve_transform_name_collisions()
