...
def _write_llms_full(app, exception):
    """Concatenate every page, in toctree order, into ``llms-full.txt`` at the HTML root."""
    ...
    with out.open("w", encoding="utf-8") as f:
        for docname in walk(app.config.master_doc):
            src = Path(app.env.doc2path(docname))
            text = src.read_text(encoding="utf-8")
            for included in re.findall(r"^\.\. include:: (\S+)$", text, flags=re.MULTILINE):
                text = text.replace(f".. include:: {included}", (src.parent / included).read_text(encoding="utf-8"))
            f.write(text)
            f.write("\n\n")
